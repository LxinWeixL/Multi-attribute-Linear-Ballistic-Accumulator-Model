---
title: "MLBA estimation on travel mode dataset (empirical study 2)"
author: "Xinwei Li\n li.xinwei@u.nus.edu"
date: "`r Sys.Date()`"
output:
...
library(coda)
library(beepr)
file_route = "...\\Empricial Study 2\\MLBA.cpp"

df = read.csv("data_merged_monitor_exp1_WL.csv")
df2 = read.csv("data_merged_laptop_exp1_WL.csv")
df = rbind(df,df2)
head(df)
...
df_pp_list = list()
basic_Info = df_info(df = df)
chosen = matrix(0,nrow = basic_Info$N,ncol =nAlt)
for (i in 1:basic_Info$N){
  chosen[i,df$rc[i]] = 1
}

df_new = data.frame(pid = matrix(t(replicate(nAlt,df$pid)),ncol = 1),
                    tid = matrix(t(replicate(nAlt,df$tid)),ncol = 1),
                    did = matrix(t(replicate(nAlt,df$did)),ncol = 1),
                    TC = basic_Info$attrs[,1],
                    TT = basic_Info$attrs[,2],
                    CL = basic_Info$attrs[,3],
                    rt = matrix(t(replicate(nAlt,df$rt)),ncol = 1) ,
                    chosen = matrix(t(chosen),ncol = 1))
df_new$pid = factor(df_new$did)
real_p = df_new%>% group_by(did) %>%nest()
...
for(type in type_set){
  ...
  df_pp = data.frame(did = NULL, real_prob = NULL,Alt = NULL, est_prob = NULL)
  for(i in 1:length(real_p$did)){
  ...
  if(type =="CRT"){
   ...
   # MLBA_rtknown_all(X = as.matrix(data_tmp[,c(3,4,5)]),beta = para$beta, zeta = para$zeta, lam1 =para$lam1, lam2 = para$lam2,  b = para$b, A =  para$A, s = para$s0, choice =rep(x,nrow(data_tmp)/nAlt) , rt = t(matrix(data_tmp$rt,nrow = 3))[,1])
    
     RCPPcdf_MLBA_CO_all(X = as.matrix(data_tmp[,c(3,4,5)]),beta = para$beta, zeta = para$zeta, lam1 =para$lam1, lam2 = para$lam2,  b = para$b, A =  para$A, s = para$s0, choice =rep(x,nrow(data_tmp)/nAlt)) 
  }
  else if (type  =="RTG"){
    MLBA_rtg_all(X = as.matrix(data_tmp[,c(3,4,5)]),beta = para$beta, zeta = para$zeta, lam1 =para$lam1, lam2 = para$lam2,  b = para$b, A =  para$A, s = para$s0, choice =rep(x,nrow(data_tmp)/nAlt) , rt = t(matrix(data_tmp$rt,nrow = 3))[,1])
  }
  else{
   RCPPcdf_MLBA_CO_all(X = as.matrix(data_tmp[,c(3,4,5)]),beta = para$beta, zeta = para$zeta, lam1 =para$lam1, lam2 = para$lam2,  b = para$b, A =  para$A, s = para$s0, choice =rep(x,nrow(data_tmp)/nAlt)) 
  }  
    })
  quantile_est = colSums(est_prob/rowSums(est_prob))/sum(est_prob/rowSums(est_prob)) 


  df_tmp = data.frame(did = rep(real_p$did[i],nAlt),
                      real_prob = Prob_real, 
                      Alt = c("Ride-hailing","Metro", "Bus"),
                      est_prob =  quantile_est)
...
##### 6.2.1.2 PP df on MNL

Please refers to section 7 below for the MNL estimation. The estimation results are stored in  "Result_table_MNL_em2.rds".

```{r}
mnl = readRDS("Result_table_MNL_em2.rds")
mnl$coefficients
basic_Info = df_info(df = df)
...
df_new = data.frame(did = matrix(t(replicate(nAlt,df$did)),ncol = 1),
                    chosen = matrix(t(chosen),ncol = 1),
                    est_prob = matrix(t(MNL_prob),ncol = 1))
df_new$pid = factor(df_new$did)
real_p = df_new%>% group_by(did) %>%nest()
...
df_pp = data.frame(did = NULL, real_prob = NULL,Alt = NULL, est_prob = NULL)
  ...
  Prob_real = rowSums(temp_chosen)/sum(rowSums(temp_chosen))
  para = para_fun(para = est_para1)
  est_prob = t(matrix(data_tmp$est_prob,nrow = nAlt))
...
for (type in type_set){
  df_pp = df_pp_list[[m]]
  plt2$pp[[m]] = 
  ggplot(data = df_pp)+geom_point(aes(x =real_prob, y = est_prob,shape = did, color = Alt),size = 3,alpha = 0.8)+
    scale_color_discrete(type = c("#D33F6A","#669157","#7A84F5"))+
    #scale_shape_manual(values = c(21,22,23,24))+
    geom_abline(aes(intercept = 0, slope = 1))+
    labs(title = bquote(.(type)~": "~R^2~" = "~.(round(df_pp$R_square,3))),
         x = "Observed Proportion", y = "Estimated Proportion",shape = "Device")+
    theme_classic()+
...
plt2$mix[[1]] = ggarrange(plotlist = plt2$pp,nrow = 1,ncol = 3, common.legend = T,legend = "bottom")
plt2$mix[[1]]
# saveRDS(plt2,"PP_plot_em2.rds")
```
...
df_long = data.frame(basic_Info$attrs,Alt = rep(1:3,basic_Info$N),chosen = choice(basic_Info$RC))
...
# LLK: -684.47; BIC :  1389.406
# -623.6
# saveRDS(m1.t,file =paste("Result_table_MNL_",df_type,".rds",sep=""))
```
...
for( i in 1:basic_Info$N){tmp[i,df$rc[i]] = 1}
