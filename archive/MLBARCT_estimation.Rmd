...
################################################## CRT
  ...
  #temp = sum(prior) +  RCPPMLBA_Lik_rtknown(attrs,beta =para[1:(nAttr)], zeta = c( para[(nAttr+1):(nAlt+nAttr-1)],0),lam1 = para[npar-2],lam2 = para[npar-1],b = para[npar], s = true_param$s0, A =para[npar]/true_param$b_A, choice = RC, rt = RT,I0 = para[npar-3]) 
...
theta_0 = chain_int
  ...
  theta_0[,(npar-2):(npar-1)] = runif(K*2, min = 0 , max = 2)
...
while (length(index)) {
  ...
  theta_0[index,zeta_index] = runif(length(index)*length(zeta_index), min =-50, max = 50)
  ...
  theta_0[index,(npar-2):(npar-1)] = runif(length(index)*2, min = 0 , max = 2)
...
Update_para = function(para){
  ...
  add = runif(npar, min = -0.001, max = 0.001)#npar
...
for (j in 1:M) {
  ...
  if (((j>round(burn_in*0.0))|(j<round(burn_in*0.9)))&(runif(1)<0.05)){
...
saveRDS(chain_list, file = "chain_list_RTG_sim(8)_May20_v2") # M =3e4, burn_in = 1.5e4, mig_rate = 0.1, all
...
# results import
```{r}
chain_list = readRDS("chain_list_CRT_sim(8)_May15_v2")
...
thin = sort(sample((burn_in*1.8):M,2e3,replace = F))
...
name = c("betaA","betaB","betaC", "zeta1","zeta2", "lam1", "lam2","b" ,"Lik")
...
gn1 = function(para){
  ...
  #temp = score_mlba_rtg_all(attrs,beta = para[1:(nAttr)],zeta = c(para[zeta_index]+true_param$zeta[3],true_param$zeta[3]),lam1 = para[npar-2],lam2 = para[npar-1],b = para[npar], s = true_param$s0, A =para[npar]/true_param$b_A, choice = RC, rt = RT)
...
Hessian1_BHHH =  function(para){
 ...
 #temp = derivative_mlba_rtg_all(attrs,beta = para[1:(nAttr)],zeta = c(para[zeta_index]+true_param$zeta[3],true_param$zeta[3]),lam1 = para[npar-2],lam2 = para[npar-1],b = para[npar], s = true_param$s0, A =para[npar]/true_param$b_A, choice = RC, rt = RT)
...
tic("MLE elapse:")
est_mle1_adj_v1 = matrix(nrow = npar+1, ncol = 5)
mle1_list_adj_v1 = list()
mle1_list_sd_v1 = list()
i = 1
k = 1
...
for (j in (1:1e4)){
  ...
  while(i<=dim(est_mle1_adj_v1)[2]){
    if(k==1){
      ...
      noise[(npar-2):(npar-1)] =  runif(2,min = -0.1,max = 0.1)
      par_value = MLE_value[-(npar+1)]+noise 
    }
...
mle1 = optimParallel(par = par_value,
      ...
      method = "L-BFGS-B",
      lower = c(0,0,0,-100,-100,0,0,0), #8
      upper = c(200,200,200,100,100,5,5,200), #8
...
w_matrix = L_vec%*%Hessian1_BHHH(mle1$par)%*%t(L_vec)
...
#1475.44 sec 
# 27458.86 sec for CO 30 ME   around 1.5h
```

```{r}
#saveRDS(est_mle1_adj_v1,"CO(8)_est_sim_ME30")
#saveRDS(mle1_list_adj_v1, "CO(8)_md_sim_ME30")
#saveRDS(mle1_list_sd_v1, "CO(8)_sd_sim_ME30")

```


```{r}
index = which.max(est_mle1_adj_v1[(npar+1),])
crt_para = est_mle1_adj_v1[,index]
crt_para[zeta_index] = crt_para[zeta_index]+true_param$zeta[3]

res_mle = data.frame(true = c(true_value,"-"),est =crt_para,se = c(mle1_list_sd_v1[[index]],"-"),gr = gn1(est_mle1_adj_v1[,index])/N,eigen_value = c(eigen(solve(-L_vec%*%Hessian1_BHHH(est_mle1_adj_v1[,index])%*%t(L_vec)))$value,max(eigen(solve(-L_vec%*%Hessian1_BHHH(est_mle1_adj_v1[,index])%*%t(L_vec)))$value)/min(eigen(solve(-L_vec%*%Hessian1_BHHH(est_mle1_adj_v1[,index])%*%t(L_vec)))$value)))
res_mle
```
...
crt_mle = optimParallel(par = par_value,
      fn =LL1, gr = gn1, 
      method = "L-BFGS-B",
      lower = c(0,0,0,-100,-100,0,0,0), #8
      upper = c(200,200,200,100,100,5,5,200), #8
