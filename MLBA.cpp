...
double RCPPcdf_rt(arma::mat X, arma::vec beta, arma::vec zeta, double lam1, double lam2, double b, double s, double A, int choice, double rt){
	...
	if(cdf<1e-10|cdf>1.1|d<(-6)) {cdf = 1e-10;}//{cdf = 1e-10;}
	return cdf;
	
}

// [[Rcpp::export]]
double RCPPcdf_rt_simple(double d, double b, double s, double A, int choice, double rt){
  double cdf_imp;
  double cdf = (1 + (b-A-d*rt)/A* normcdf((b-A-d*rt)/(s*rt)) - (b-d*rt)/A* normcdf((b-d*rt)/(s*rt)) + s*rt/A* normpdf((b-A-d*rt)/(s*rt))-s*rt/A* normpdf((b-d*rt)/(s*rt)))/normcdf(d/s);
...
double RCPPpdf_rt(arma::mat X, arma::vec beta, arma::vec zeta, double lam1, double lam2, double b, double s, double A, double choice, double rt){
	...
	if(pdf_imp<1e-10) {pdf_imp = 1e-10;}//{pdf_imp = 0;}
	if(pdf_imp>10) {pdf_imp = 1e-10;}//{pdf_imp = 0;}
	if(d<=-6) {pdf_imp = 1e-10;}//{pdf_imp = 0;}
	return pdf_imp;
	
}

// [[Rcpp::export]]
double RCPPpdf_rt_simple(double d, double b, double s, double A, double choice, double rt){
...
double RCPPpdf_MLBA(arma::mat X, arma::vec beta, arma::vec zeta, double lam1, double lam2, double b, double s, double A,double choice, double rt){
	...
	if (pdf_MLBA_imp<1e-10) {pdf_MLBA_imp = 1e-10;}//{pdf_MLBA_imp = 0;}
	return pdf_MLBA_imp;
}
